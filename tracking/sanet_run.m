function [ result ] = sanet_run(images, region, net, rnet, display)
...
[net_conv, net_fc, opts] = sanet_init(img, net);
RNT = load(rnet);
RNT = RNT.RNT;
...
if(opts.bbreg)
    ...
    feat_conv = sanet_features_convX(net_conv, RNT, img, pos_examples, opts);
...
feat_conv = sanet_features_convX(net_conv, RNT, img, examples, opts);
pos_data = feat_conv(:,:,:,pos_idx);
neg_data = feat_conv(:,:,:,neg_idx);

%% Learning CNN
fprintf('  training networks...\n');
net_fc = sanet_finetune_hnm(net_fc,pos_data,neg_data,opts,...
    'maxiter',opts.maxiter_init,'learningRate',opts.learningRate_init);

%% Initialize displayots
if display
    figure(1);
    set(gcf,'Position',[200 100 size(img, 2) size(img, 1)],'MenuBar','none','ToolBar','none');
    
    hd = imshow(img,'initialmagnification','fit'); hold on;
    rectangle('Position', targetLoc, 'EdgeColor', [0 0 1], 'Linewidth', 5);
    set(gca,'position',[0 0 1 1]);
    
    text(20,20,['# ' num2str(1)],'Color','r', 'HorizontalAlignment', 'left', 'FontWeight','bold', 'FontSize', 23);
    hold off;
    drawnow;
end
...
for To = 2:nFrames;
    ...
    feat_conv = sanet_features_convX(net_conv, RNT, img, samples, opts);
    
    % evaluate the candidates
    feat_fc = sanet_features_fcX(net_fc, feat_conv, opts);
...
%         fprintf('\n');
    ...
    if display
        ...
        rectangle('Position', result(To,:), 'EdgeColor', [0 0 1], 'Linewidth', 3);
        set(gca,'position',[0 0 1 1]);
        
        text(20,20,['# ' num2str(To)],'Color','r', 'HorizontalAlignment', 'left', 'FontWeight','bold', 'FontSize', 23); 
        hold off;
        drawnow;
    end
end