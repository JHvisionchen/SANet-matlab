function [net, info] = sanet_train(net, roidb, getBatch, varargin)
opts.batch_frames = 8 ;
opts.batchSize    = 128 ;
opts.batch_pos    = 32;
opts.batch_neg    = 96;
...
for t=1:opts.numCycles
    ...
    % set learning rate for rnn. Note that the learning for rnn is
    % different from that for convolutional neural networks.
    net.rnn_param.lr = net.rnn_param.initLr * (net.rnn_param.lrdecay^(t-1));
    net.rnn_param1.lr = net.rnn_param1.initLr * (net.rnn_param1.lrdecay^(t-1));
    net.rnn_param2.lr = net.rnn_param2.initLr * (net.rnn_param2.lrdecay^(t-1));
    ...
    for seq_id=1:K
        ...
        res = sanet_simplenn(net, im, 1, one, res, 'conserveMemory', opts.conserveMemory, 'sync', opts.sync) ;
